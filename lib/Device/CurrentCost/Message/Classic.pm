package Device::CurrentCost::Message::Classic;
...
sub device_type {
  ...
  my $pm = $self->parsed_message();
  $self->{device_type} =
    ref $self->parsed_message->{src} eq 'HASH'
    && exists $self->parsed_message->{src}->{name}
        ? CURRENT_COST_CLASSIC : CURRENT_COST_ENVY;
}
...
sub parsed_message { 
    my $self = shift;
    return $self->{_parsed_message} if exists $self->{_parsed_message};
    my $xml;
   # eval { 
        $xml = XMLin($self->message);
   # };
    
    if ($@) {
	$self->{_parsed_message} = undef;
    }
    else {
	$self->{_parsed_message} = $xml;
    }
